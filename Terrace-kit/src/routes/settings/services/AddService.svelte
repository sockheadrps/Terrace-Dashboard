<script>
	...
	import { newService } from '$lib/stores/serviceStore';
	...
	<div class="container">
		<div class="icon__area">
			<div class="icon__searchbar">
				<Icon currentService.icon="ic:baseline-search" />
				<input
					type="text"
					class="w-full rounded-md text-original-muted bg-gray-700 outline-none pl-2 focus:bg-original-input-hover placeholder:focus:text-[rgba(100,100,100,.70)] hover:bg-gray-800 transition-colors mr-2"
				...
				<button
					...
					type="submit"
					class="submit"
					on:click|preventDefault={() => newService(currentService)}
					on:click={close}
					>Save
				</button>
			</div>
		...
		<div class="results">
			{#if currentService.icon && currentService.icon !== ''}
				<button class="selected" on:click={() => (currentService.icon = '')}>
					<Icon icon={currentService.icon} />
				</button>
			{/if}
			{#each icons as ic}
				{#if ic !== currentService.icon}
					<button style="color:inherit" on:click={() => (currentService.icon = ic)}>
						<Icon icon={ic} />
					</button>
				{/if}
			{/each}
