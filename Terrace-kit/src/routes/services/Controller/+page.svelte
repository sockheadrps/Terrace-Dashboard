<script lang="ts">
    ...
    function setSlider() {
        websocketSend('DATA-REQUEST', {'TARGET-CLIENT': "Controller", 'QUERY': 'level', 'TARGET': selected})
    }
    ...
    $: if (sliderValue !== undefined) {
        websocketSend("SERVICE-DATA", {'TARGET-CLIENT': "Controller", "LEVEL": sliderValue, "MEDIA": selected})
    }
...
{#if ready==true}
    <div class="flex flex-col justify-center mb-20" in:fade>
        <div class="flex flex-row justify-center opacity-60 hover:opacity-95">
            <select class="w-1/3 h-12 outline-none rounded-2xl border-2 bg-original-table-header border-original-dark text-original-muted text-2xl text-center cursor-pointer" bind:value={selected}
            on:change={() => setSlider()}>
        ...
        <div class="flex flex-row justify-center mt-10 items-center">
            <!-- Previous button -->
                <button class="flex items-center justify-center p-3 h-24 w-24 border-4 m-2 text-5xl rounded-full border-original-dark text-original-muted gradient backdrop:blur-sm focus:outline-none" on:click|preventDefault={() => websocketSend("SERVICE-DATA", {'TARGET-CLIENT': "Controller", "COMMAND": "previous"})}>
                    <Icon icon="fluent:previous-16-regular" />
                </button>
                
                <!-- Play/Pause -->
                <button class="flex items-center justify-center p-3 h-28 w-28 border-4 m-2 text-6xl
                    rounded-full border-original-dark text-original-muted gradient backdrop:blur-sm
                    focus:outline-none" on:click|preventDefault={() =>{ websocketSend("SERVICE-DATA", {'TARGET-CLIENT': "Controller", "COMMAND": "play-pause"}); playing = !playing}}>
                    {#if playing}
                        <div class="absolute" in:fade={{ delay: 250, duration: 250 }} out:fade={{ duration: 250 }}>
            ...
            <!-- Next button -->
                    <button class="flex items-center justify-center h-24 w-24 p-3 border-4 m-2 text-5xl rounded-full border-original-dark text-original-muted gradient     backdrop:blur-sm focus:outline-none" on:click|preventDefault={() => websocketSend("SERVICE-DATA", {'TARGET-CLIENT': "Controller", "COMMAND": "next"})}>
